...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        values = do_lambda_form(Pair(formals, expressions.rest), env)
        env.define(new_target, values)
        return new_target
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            return scheme_eval(expressions.first, env, True)
        indicate = scheme_eval(expressions.first, env)
        if scheme_falsep(indicate):
            return False
        expressions = expressions.rest
    return indicate
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    tempor_bindings = bindings
    while tempor_bindings is not nil:
        pair = tempor_bindings.first
        check_form(pair, 2, 2) #both max and min are 2 since each pairing has a symbol and a corresponding expression
        tempor_bindings = tempor_bindings.rest
    symbols = bindings.map(lambda x: x.first)
    values = bindings.map(lambda x: scheme_eval(x.rest.first, env))
