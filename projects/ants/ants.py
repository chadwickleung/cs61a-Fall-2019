    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if insect.is_TrueQueen is True:
                    return
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.scared_yet or self.can_go_backward == False:
            destination = self.place.exit
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        tempor_place = self.place
        count = 0
        while tempor_place is not beehive:
            bee_selected = random_or_none(tempor_place.bees)
            if bee_selected and count >= self.min_range and count <= self.max_range:
                return bee_selected
            tempor_place = tempor_place.entrance
            count += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        damage_on_bees = amount
        list_of_bees_in_sameplace = list(self.place.bees)
        Ant.reduce_armor(self, amount)
        if self.armor <= 0:
            damage_on_bees = amount + self.damage
        for bee in list_of_bees_in_sameplace:
            Insect.reduce_armor(bee, damage_on_bees)
        # END Problem 5
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        copy_of_bee_list = list(self.place.bees)
        for x in copy_of_bee_list:
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return self.contained_ant == None and other.is_container is False
        # END Problem 9
...
class ScubaThrower(ThrowerAnt):
    "A ThrowerAnt that can be placed underwater"
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_TrueQueen:
            ...
            temporary_place = self.place.exit
            while temporary_place != None:
                if temporary_place.ant:
                    if isinstance(temporary_place.ant, BodyguardAnt):
                        if temporary_place.ant.contained_ant:
                            if temporary_place.ant.contained_ant.doubling_yet is False:
                                temporary_place.ant.contained_ant.damage = temporary_place.ant.contained_ant.damage*2
                                temporary_place.ant.contained_ant.doubling_yet = True
                    if temporary_place.ant.doubling_yet is False:
                        temporary_place.ant.damage = temporary_place.ant.damage*2
                        temporary_place.ant.doubling_yet = True
                temporary_place = temporary_place.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        if self.is_TrueQueen and self.armor <= 0:
            bees_win()
...
def make_scare(action, bee):
    ...
    """
    # BEGIN Problem EC
    bee.scared_yet = False
    bee.can_go_backward = True
    def new_scary_action(colony):
        return action(colony)
    return new_scary_action
    # END Problem EC

def apply_effect(effect, bee, duration):
    """Apply a status effect to a BEE that lasts for DURATION turns."""
    # BEGIN Problem EC
    initial_action = bee.action
    modified_action = effect(initial_action, bee)
    
    def helper_effect(colony):
        nonlocal duration
        if duration > 0:
            modified_action(colony)
        else:
            bee.scared_yet = True
            initial_action(colony)
        duration -= 1
    bee.action = helper_effect
